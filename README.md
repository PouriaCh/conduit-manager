...
- **Live Dashboard** — Real-time stats showing CPU, RAM, connections, and upload/download
- **Per-Container Status** — Individual monitoring for each container with detailed metrics
- **Background Tracker** — Captures network traffic every 15 seconds with country-level GeoIP statistics
- **Live Peers by Country** — Full-screen display showing TOP 10 countries by traffic volume
- **Telegram Notifications** — Automated reports, alerts (CPU/RAM/down), and bot commands
- **Bot Commands** — Remote management via Telegram: `/status`, `/uptime`, `/containers`, `/restart_N`, `/stop_N`, `/start_N`
...
- **Background Tracker Service** — 24/7 network monitoring with country-level statistics
...
- **Live Connection Stats** — Real-time monitoring with 5-second refresh
- **macOS-Specific Optimizations** — Robust Docker resolution, PATH fixes for nohup services
...
- **tcpdump Requires sudo** — Live peers map auto-prompts for sudo when needed
- **Nohup Services** — Tracker and Telegram run via nohup (not systemd)

### Feature Adaptations
- **Live Map (`conduit peers`)** — Requires sudo on macOS, auto-prompts when accessed via menu
- **Telegram `/peers` Command** — Not available (unreliable peer tracking in background notifier)
- **Zero-Peers Alert** — Disabled on macOS (same tracking limitation as `/peers`)
- **GeoIP Database** — Uses free DB-IP Lite (no account needed)

### Known Limitations
- Background notifier cannot reliably get peer counts from `docker logs` on macOS
- Live Map works but requires sudo, so not advertised in Telegram bot
- No systemd service management (services run via nohup)
